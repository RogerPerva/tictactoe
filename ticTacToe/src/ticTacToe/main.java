	...
	static class InitialMenu {
		...
		private EnumBoardSize selectBoardSize() {
			...
			while (selectedBoardSize == null) {
				...
				System.out.print("Enter your option (1-" + EnumBoardSize.values().length + "): "); // Request "board
																									// size" option from
																									// the user
				int option1 = scanner.nextInt();

				if (option1 >= 1 && option1 <= EnumBoardSize.values().length) {// Validates that the selected option
																				// exists in the ENUM
					selectedBoardSize = EnumBoardSize.values()[option1 - 1];
					System.out.println("You have selected: " + selectedBoardSize.getDescription() + " with size "
							+ selectedBoardSize.getBoardSize());
				} else {
		...
		private EnumGameType selectGameType() {
			while (selectedGameType == null) {
				...
				if (option2 >= 1 && option2 <= EnumGameType.values().length) {
					...
					System.out.println("You have selected: " + selectedGameType.getDescriptionGameType() + " with type "
							+ selectedGameType.getValueGameType());
				} else {
